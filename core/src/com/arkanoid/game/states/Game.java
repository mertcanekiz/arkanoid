...
public class Game extends GameState {
    ...
    public Game() {
        level = new Level("level-3.txt");
        paddle = new Paddle();
        ball = new Ball(new Vector2(50, 50));
        font = new BitmapFont(Gdx.files.internal("retro.fnt"), Gdx.files.internal("retro.png"), false);
    ...
    public void render(SpriteBatch sb) {
        ...
        font.draw(sb, "HIGH", ArkanoidGame.WIDTH + 5, ArkanoidGame.HEIGHT - 10);
        font.draw(sb, "SCORE", ArkanoidGame.SCREEN_WIDTH - w * 6 + 2, ArkanoidGame.HEIGHT - 10 - h);
        font.setColor(Color.WHITE);
        font.draw(sb, "50000", ArkanoidGame.SCREEN_WIDTH - w * 6 + 2, ArkanoidGame.HEIGHT - 10 - h * 2);

        font.setColor(new Color(0xb53121ff));
        font.draw(sb, "1UP", ArkanoidGame.WIDTH + w / 2, ArkanoidGame.HEIGHT - 50);
        font.setColor(Color.WHITE);
        font.draw(sb, "0", ArkanoidGame.SCREEN_WIDTH - w * 2, ArkanoidGame.HEIGHT - 50 - h);

        for (int i = 0; i < paddle.lives; i++) {
            float x = (i % 3) * (Paddle.WIDTH / 2f + 2);
            float y = (i / 3) * Paddle.HEIGHT / 2f;
            sb.draw(Paddle.img_normal, ArkanoidGame.WIDTH + 5 + x, ArkanoidGame.HEIGHT - 120 + y, Paddle.WIDTH / 2f, Paddle.HEIGHT / 2f);
        }

        font.setColor(new Color(0xb53121ff));
        font.draw(sb, "ROUND", ArkanoidGame.SCREEN_WIDTH - w * 6, ArkanoidGame.HEIGHT - 180);
        font.setColor(Color.WHITE);
        font.draw(sb, "1", ArkanoidGame.SCREEN_WIDTH - w  - 7, ArkanoidGame.HEIGHT - 180 - h);
    }
